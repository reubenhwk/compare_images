...
void InterpKeyPoint(flimage * dogs, int s, int r, int c,
...
void MakeKeypoint(const flimage & grad, const flimage & ori, float octSize, float octScale,
...
inline void AddSample(float index[IndexSize][IndexSize][OriSize], keypoint & key,
...
void OctaveKeypoints(flimage & image, float octSize, keypointslist & keys, siftPar & par)
{
	...
	for (int i = 0; i < par.Scales + 2; i++) {
		...
		combine(dogs[i].getPlane(), 1.0f, blur[i + 1].getPlane(), -1.0f, dogs[i].getPlane(),
			dogs[i].nwidth() * dogs[i].nheight());
	}
...
void FindMaxMin(flimage * dogs, flimage * blur, float octSize, keypointslist & keys, siftPar & par)
{
	...
	for (int s = 1; s < par.Scales + 1; s++) {
		...
		compute_gradient_orientation(blur[s].getPlane(), grad.getPlane(), ori.getPlane(), blur[s].nwidth(),
					     blur[s].nheight());
...
*/
					if (LocalMaxMin(val, dogs[s - 1], r, c) && LocalMaxMin(val, dogs[s], r, c)
						...
						InterpKeyPoint(dogs, s, r, c, grad, ori, map, octSize, keys, 5, par);
		    ...
		    float octSize, keypointslist & keys, int movesRemain, siftPar & par)
{
	...
	if (movesRemain > 0 && (newr != r || newc != c)) {
		InterpKeyPoint(dogs, s, newr, newc, grad, ori, map, octSize, keys, movesRemain - 1, par);
		return;
	}
	...
	if (fabs(offset[0]) > 1.5 || fabs(offset[1]) > 1.5 || fabs(offset[2]) > 1.5 || fabs(peakval) < par.PeakThresh) {
		if (DEBUG)
	...
	//if (UseHistogramOri)
	AssignOriHist(grad, ori, octSize, octScale, r + offset[1], c + offset[2], keys, par);
	//else
	//      AssignOriAvg(
...
float FitQuadratic(float offset[3], flimage * dogs, int s, int r, int c)
{
	...
	H[0][1] = H[1][0] = (((*dog2) (c, r + 1) - (*dog2) (c, r - 1)) - ((*dog0) (c, r + 1) - (*dog0) (c, r - 1))) / 4.0;
	H[0][2] = H[2][0] = (((*dog2) (c + 1, r) - (*dog2) (c - 1, r)) - ((*dog0) (c + 1, r) - (*dog0) (c - 1, r))) / 4.0;
...
//      float   hist[par.OriBins], distsq, dif, gval, weight, angle, interp;
	...
	int row = (int)(octRow + 0.5), col = (int)(octCol + 0.5), rows = grad.nheight(), cols = grad.nwidth();
	...
	for (int i = 0; i < par.OriBins; i++) {
		...
		if (hist[i] > hist[prev] && hist[i] > hist[next] && hist[i] >= par.OriHistThresh * maxval) {
			...
			MakeKeypoint(grad, ori, octSize, octScale, octRow, octCol, angle, keys, par);
		}
	...
	for (int i = -iradius; i <= iradius; i++) {
		for (int j = -iradius; j <= iradius; j++) {
			...
			if (rx > -1.0 && rx < (float)IndexSize && cx > -1.0 && cx < (float)IndexSize)
				AddSample(index, key, grad, ori, irow + i, icol + j, rpos, cpos, rx, cx, par);
		}
	}
}
		      ...
		      float r, float c, float rpos, float cpos, float rx, float cx, siftPar & par)
{
	...
	float sigma = par.IndexSigma * 0.5 * IndexSize, weight = exp(-(rpos * rpos + cpos * cpos) / (2.0 * sigma * sigma)),
//              mag    = weight *  grad(c,r);
...
*/
inline void PlaceInIndex(float index[IndexSize][IndexSize][OriSize], float mag, float ori, float rx, float cx, siftPar & par)
{
...
//      if (abs(k1.x - k2.x) > par.MatchXradius || abs(k1.y - k2.y) > par.MatchYradius) return tdist;
	...
	if (ABS(k1.x - k2.x) > par.MatchXradius || ABS(k1.y - k2.y) > par.MatchYradius)
		return tdist;
