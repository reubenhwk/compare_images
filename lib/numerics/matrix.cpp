...
/// Access the coefficient on the \a i-th row, \a j-th column.
	template < typename T > inline T matrix < T >::operator()(int i, int j)const {
...
/// Access the coefficient on the \a i-th row, \a j-th column.
	...
	template < typename T > inline T matrix < T >::operator()(int i)const {
		assert(i >= 0 && i < nElements());
		return p[i];
	}
	template < typename T > inline T & matrix < T >::operator() (int i) {
		assert(i >= 0 && i < nElements());
		return p[i];
	}
...
/// Assign all coefficients to the value \a a.
	template < typename T > inline void matrix < T >::operator=(T a) {
		for (int i = nElements() - 1; i >= 0; i--)
			p[i] = a;
	}
...
/// \param a a scalar.
	template < typename T > matrix < T > matrix < T >::operator*(T a) const {
		matrix < T > prod(m_rows, m_cols);
		for (int i = nElements() - 1; i >= 0; i--)
			prod.p[i] = a * p[i];
		return prod;
	}
/// Multiply a matrix by scalar./// \param a a scalar.
	template < typename T >
...
/// Divide a matrix by scalar./// \param a a scalar.
	template < typename T >
	void matrix < T >::operator/=(T a) {
		*this *= (T) 1 / a;
	}
...
/// Symmetrize upper part of matrix.
	template < typename T > void matrix < T >::symUpper() {
...
/// Symmetrize lower part of matrix.
	template < typename T > void matrix < T >::symLower() {
		assert(m_rows == m_cols);
		for (int i = 1; i < m_rows; i++) {
	...
	template < typename T > vector < T > matrix < T >::diag()const {
		assert(m_rows == m_cols);
		vector < T > t(m_rows);
...
/// Matrix made of ones.
	template < typename T > matrix < T > matrix < T >::ones(int m, int n) {
		matrix < T > M(m, n);
		for (int i = M.nElements() - 1; i >= 0; i--)
			M.p[i] = (T) 1;
		return M;
...
/// Extract the rows of index in [i0,i1]./// \param i0 first row/// \param i1 last row
	template < typename T >
	matrix < T > matrix < T >::copyRows(int i0, int i1)const {
		return copy(i0, i1, 0, lastCol());
	}
/// Paste a matrix in another one, at position (\a i0,\a j0)/// \param i0 first row where to paste in/// \param j0 first column where to paste in/// \param matrix to paste
	template < typename T >
...
/// Copy column number \a j.
	template < typename T > vector < T > matrix < T >::col(int j)const {
		assert(j >= 0 && j < m_cols);
		vector < T > c(m_rows);
		const T *in = p + j;
		for (int i = 0; i < m_rows; i++) {
...
/// Copy row number \a i.
	template < typename T > inline matrix < T > matrix < T >::row(int i)const {
		return copy(i, i, 0, lastCol());
	}
	template < class T > void swap(matrix < T > &A, matrix < T > &B) {
		int i = A.m_rows;
		A.m_rows = B.m_rows;
		B.m_rows = i;
		i = A.m_cols;
	...
	template < typename T > void matrix < T >::swapRows(int i0, int i1) {
		assert(0 <= i0 && i0 < m_rows && 0 <= i1 && i1 < m_rows);
		T *row0 = p + i0 * m_cols;
		T *row1 = p + i1 * m_cols;
	...
	template < typename T > void matrix < T >::swapCols(int j0, int j1) {
		assert(0 <= j0 && j0 < m_cols && 0 <= j1 && j1 < m_cols);
		T *col0 = p + j0;
		T *col1 = p + j1;
...
/// The matrix is scanned row by row.
	template < typename T > void matrix < T >::write(T * vect) const {
		for (int i = nElements() - 1; i >= 0; i--)
			vect[i] = p[i];
	}
	template < typename T > void matrix < T >::alloc(int m, int n) {
		assert(m > 0 && n > 0);
		m_rows = m;
		m_cols = n;
		p = new T[m * n];
	...
	template < typename T > inline void matrix < T >::free() {
		delete[]p;
		p = NULL;
	}

	template < typename T > inline int matrix < T >::nElements() const {
