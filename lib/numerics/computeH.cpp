...
// Add two corresponding points, type involving at most 4 parameters
	void ComputeH::add_4parameters(float x, float y, float X, float Y,
				       float w) {
		double *A = Ann, *B = Bn;
...
// Add corresponding lines of equation ux + by + x = 0
	void ComputeH::add(float x, float y, float z, float X, float Y, float Z,
		...
		double x2 = x * x, y2 = y * y, z2 = z * z, xy = x * y, xz =
		    x * z, yz = y * z;
		double X2 = X * X, Y2 = Y * Y, Z2 = Z * Z, XY = X * Y, XZ =
		    X * Z, YZ = Y * Z;
		double *A = Ann, *B = Bn;
...
// Wrap vector of unknowns `v' into structure `map'
	void ComputeH::wrap(Homography & h, const vector < double >&v) const {
		int i = 0;
		 ...
		 h.mat() (0, 1) = (_type == Translation || _type == Zoom
				   || _type == GeneralZoom) ? 0 : v(i++);
		if (n >= 6) {
			h.mat()(1, 0) = v(i++);
			h.mat() (1, 1) = v(i++);
		} else {
			h.mat()(1, 0) = -h.mat()(0, 1);
			h.mat()(1, 1) =
			    (_type == GeneralZoom) ? v(i++) : h.mat()(0, 0);
		}
		h.mat()(0, 2) = v(i++);
		h.mat()(1, 2) = v(i++);
		if (_type == Projective) {
	...
	void ComputeH::unwrap(const Homography & h, vector < double >&v) const {
		...
		if (_type != Translation) {
			...
			if (_type != Zoom) {
				...
				if (_type == GeneralZoom || _type == Affine
				    || _type == Projective)
					v(i++) = h.mat()(1, 1);
			}
		}
...
// LSE for rotation: solve linear system under quadratic constraint
	bool ComputeH::compute_rotation(vector < double >&B) const {
		...
		// Test first solution
		double lambda1 =
		    (Ann[2] * Ann[2] + Ann[3] * Ann[3] + root) / Ann[15];
		 B(0) /= root;
		 B(1) /= root;
		 B(2) =
		    (-Ann[2] * Bn[0] - Ann[3] * Bn[1] + lambda1 * Bn[2]) / root;
		 B(3) =
		    (-Ann[3] * Bn[0] + Ann[2] * Bn[1] + lambda1 * Bn[3]) / root;
		float v1 = q_error(B);
		...
		double lambda2 =
		 ...
		 C(1) = -B(1);
		 C(2) =
		    -(-Ann[2] * Bn[0] - Ann[3] * Bn[1] +
		      lambda2 * Bn[2]) / root;
		 C(3) =
		    -(-Ann[3] * Bn[0] + Ann[2] * Bn[1] +
		      lambda2 * Bn[3]) / root;
...
// origin is their centroid and their mean square distance (to it) is 2
	bool ComputeH::normalize(Normalization & left,
		...
		double s2 = right.s * right.s, sS = right.s * left.s, S2 =
		    left.s * left.s;
		...
		double a0 = A(0, 0), a1 = A(0, 1), a6 = A(0, 6), a7 =
		    A(0, 7), a9 = A(1, 1);
		double a15 = A(1, 7), a22 = A(2, 6), a23 = A(2, 7), a31 =
		    A(3, 7);
		 ...
		 A(0, 4) = 0;
		 A(0, 6) =
		    right.s * (sS * A(0, 6) - right.s * left.x * a0 -
			       left.s * right.x * b0) +
		    w * right.x * left.x * right.x - right.x * B(0);
		 A(0, 7) =
		    right.s * (sS * A(0, 7) - right.s * left.x * a1 -
			       left.s * right.x * b1) +
		 ...
		 A(1, 6) = A(0, 7);
		 A(1, 7) =
		    right.s * (sS * A(1, 7) - right.s * left.x * a9 -
			       left.s * right.y * b1) +
		 ...
		 A(2, 5) = 0;
		 A(2, 6) =
		    right.s * (sS * A(2, 6) - right.s * left.y * a0 -
			       left.s * right.x * b2) +
		    w * right.x * left.y * right.x - right.x * B(2);
		 A(2, 7) =
		    right.s * (sS * A(2, 7) - right.s * left.y * a1 -
			       left.s * right.x * b3) +
		 ...
		 A(3, 6) = A(2, 7);
		 A(3, 7) =
		    right.s * (sS * A(3, 7) - right.s * left.y * a9 -
			       left.s * right.y * b3) +
		 ...
		 A(6, 6) =
		    s2 * (S2 * A(6, 6) -
			  2 * left.s * (left.x * a6 + left.y * a22) +
			  a0 * norm) - 2 * right.x * (B(6) + w * right.x);
		 A(6, 7) =
		    s2 * (S2 * A(6, 7) -
			  2 * left.s * (left.x * a7 + left.y * a23) +
			  a1 * norm) - right.x * B(7) - right.y * B(6) -
		    2 * w * right.x * right.y;

		 A(7, 7) =
		    s2 * (S2 * A(7, 7) -
			  2 * left.s * (left.x * a15 + left.y * a31) +
			  a9 * norm) - 2 * right.y * (B(7) + w * right.y);
		 return true;
	}
// `l' (left) and 'r' (right) representing zoom-translation normalizations,// and `B' the parameters of a projective motion,// compute l^-1 B r
