...
/// Resolution by LU decomposition with pivot.
	    bool solveLU(const matrix < flnum > &A,
				 const vector < flnum > &B,
				 vector < flnum > &X) {
		X = B;
		return solveLU(A, X);
	}
...
/// points this would be sqrt(2) times the returned value.
	flnum MinLM::minimize(vector < flnum > &P,
			      const vector < flnum > &yData, flnum targetRMSE,
			      int maxIters) {
		...
		for (iterations = 0; iterations < maxIters && error > errorMax;
		     iterations++) {
			matrix < flnum > H(JtJ);
