...
void unique_match1(matchingslist & seg_in, matchingslist & seg_out, vector < vector < float > >&Minfoall_in,
...
void clean_match1(matchingslist & seg_in, matchingslist & seg_out, vector < vector < float > >&Minfoall_in,
		  vector < vector < float > >&Minfoall_out)
{
	int i1, i2;
	float x1_in, x2_in, y1_in, y2_in, x1_out, x2_out, y1_out, y2_out;
	...
	if (sum_flag > 0) {
		...
		printf
		    ("Warning: all matches are redundant and are thus removed! This step of match cleaning is short circuited. (Normally this should not happen...)\n");
	}
...
void clean_match2(matchingslist & seg_in, matchingslist & seg_out, vector < vector < float > >&Minfoall_in,
		  vector < vector < float > >&Minfoall_out)
{
	int i1, i2;
	float x1_in, x2_in, y1_in, y2_in, x1_out, x2_out, y1_out, y2_out;
...
void compensate_affine_coor(matching & matching1, int w1, int h1, int w2, int h2, float t1, float t2, float Rtheta,
			    float t_im2_1, float t_im2_2, float Rtheta2)
{
	float x_ori, y_ori;
...
#pragma omp parallel for private(rr2)
				...
				for (int rr2 = 1; rr2 <= num_rot1_2; rr2++) {
					...
					if (verb) {
						printf
						    ("t1=%.2f, theta1=%.2f, num keys1 = %d, t2=%.2f, theta2=%.2f, num keys2 = %d, num matches=%d\n",
						     t, theta, (int)keypoints1.size(), t_im2, theta2, (int)keypoints2.size(),
						     (int)matchings1.size());
					}

					/* Store the matches */
					if (matchings1.size() > 0) {
						matchings_vec[tt - 1][rr - 1][tt2 - 1][rr2 - 1] =
...
//                      float nfa = orsa(w1, h1, match_coor, index, t_value_orsa, verb_value_orsa, n_flag_value_orsa, mode_value_orsa, stop_value_orsa);
			float nfa =
			    orsa((w1 + w2) / 2, (h1 + h2) / 2, match_coor, index, t_value_orsa, verb_value_orsa,
				 n_flag_value_orsa, mode_value_orsa, stop_value_orsa);
			...
			if (nfa < nfa_max) {
				...
				cout << "The two images match! " << matchings.
				    size() << " matchings are identified. log(nfa)=" << nfa << "." << endl;
				return 0;
			} else {
				matchings.clear();
				Minfoall.clear();
