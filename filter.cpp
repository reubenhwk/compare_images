...
float *directional_gauss_filter(float xsigma, float ysigma, float angle, int *kwidth, int *kheight)
{
	...
	for (int y = -l2; y <= l2; y++)
		for (int x = -l2; x <= l2; x++) {
			...
			kernel[(y + l2) * ksize + x + l2] =
			    exp(-(ax * ax) / (2.0f * xsigma2) - (ay * ay) / (2.0f * ysigma2));
...
void remove_outliers(float *igray, float *ogray, int width, int height)
{
	...
	for (int x = bloc; x < width - bloc; x++)
		for (int y = bloc; y < height - bloc; y++) {
			...
			if (countmin == bsize || countmax == bsize)
				ogray[l] = green;
			else
				ogray[l] = igray[l];
...
void separable_convolution(float *u, float *v, int width, int height, float *xkernel, int xsize, float *ykernel, int ysize,
			   int boundary)
{
...
void fast_separable_convolution(float *u, float *v, int width, int height, float *xkernel, int xsize, float *ykernel,
				int ysize, int boundary)
{
	copy(u, v, width * height);
		...
		if (sigma > 0.0) {
			...
			for (i = 0; i < width; i++)
				for (j = 0; j < height; j++) {
					...
					laplacian[j * width + i] =
					    -4.0 * input[width * j + i] + input[width * j + im] + input[width * j + i1] +
					    input[width * jm + i] + input[width * j1 + i];
				}
		}
