...
void binarize(float *u, float *v, float value, int inverse, int size)
{
	for (int i = 0; i < size; i++) {
		...
		if (u[i] >= value && !inverse)
			v[i] = 255.0;
		else if (u[i] <= value && inverse)
			v[i] = 255.0;
		else
			v[i] = 0.0;
	}
...
float *histo(float *input, float *iminim, float *imaxim, int *n, float *s, int size, char flag)
{
	...
	if (iminim)
		...
		minim = min(input, NULL, size);

	float maxim;
	if (imaxim)
		maxim = *imaxim;
	else
		maxim = max(input, NULL, size);
...
void compute_gradient_orientation(float *igray, float *grad, float *ori, int width, int height)
{
	...
	for (r = 0; r < rows; r++)
		for (c = 0; c < cols; c++) {
			...
			if (grad)
				grad[r * cols + c] = (float)sqrt((double)(xgrad * xgrad + ygrad * ygrad));
			if (ori)
				ori[r * cols + c] = (float)atan2(-(double)ygrad, (double)xgrad);
...
void gray(float *red, float *green, float *blue, float *out, int width, int height)
{
	for (int i = width * height - 1; i > 0; i--)
		out[i] = (red[i] + green[i] + blue[i]) / 3.0;
}
